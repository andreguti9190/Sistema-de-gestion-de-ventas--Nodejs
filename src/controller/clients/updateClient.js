...
import { updateClient } from "../../model/clients.model.js"
import { validateUUID, validateClientSoft } from "../validation/client.validation.js"
...
const updateClientController = async (req, res) => {
    ...
    const validInformation = await validateClientSoft(name, email);

    if (!validInformation) return res.status(400).json({ error: true, msg: "information format is incorrect" })

    let data = await updateClient(id, { name, email });
